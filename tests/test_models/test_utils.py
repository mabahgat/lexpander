...
from models.utils import get_model_by_name_or_none
...
def test_get_model_by_name_or_none(tmp_path):
	...
	bert = get_model_by_name_or_none(name='bert', exp_name='test_exp', dataset=dataset)
	assert type(bert) == BertClassifier

	blah = get_model_by_name_or_none(name='blah', exp_name='test_exp')
	assert blah is None
